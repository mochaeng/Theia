...
import { enforceLogin, fetchWithAuth } from '@/oidc'
...
function DocumentUploadWithProgress() {
  ...
  const {
    ...
    connect,
    disconnect,
  } = useDocumentProgress(handleProcessingComplete)

  const uploadDocument = async (file: File) => {
    const formData = new FormData()
    formData.append('file', file)

    const response = await fetchWithAuth(UPLOAD_ENDPOINT, {
      method: 'POST',
      body: formData,
    })
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (data) => {
      console.log(`upload successful for document '${data.documentID}'`)
  ...
  const isProcessing = connectionStatus !== 'DISCONNECTED'
